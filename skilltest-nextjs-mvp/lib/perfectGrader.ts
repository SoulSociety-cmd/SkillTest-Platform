...
/**
 * PERFECT AI GRADING SYSTEM FOR SKILLTEST (TypeScript)
 * GPT-4o System Prompt for Professional Code Evaluation
 */

export interface PerfectGradingResult {
  success: boolean;
  technicalScore: number;
  codeQuality: number;
  problemSolving: number;
  overall: number;
  feedback: string; // Exactly 3 paragraphs
...
export async function gradeSubmission(input: GradingInput): Promise<PerfectGradingResult> {
  const { code, testResults = { passed: 0, total: 0 }, timeSpent = 0, timeMax = 0, language = 'JavaScript' } = input;
  ...
  try {
    ...
    const result = JSON.parse(jsonStr) as PerfectGradingResult;

    // TypeScript validation
    const required: (keyof PerfectGradingResult)[] = ['technicalScore', 'codeQuality', 'problemSolving', 'overall', 'feedback', 'hireRecommendation'];
    for (const key of required) {
      if (result[key] === undefined) {
        throw new Error(`Missing ${String(key)} in AI response`);
      }
    }
    ...
    const testScore = total > 0 ? (passed / total) * 10 : 0;
    const timeFactor = timeMax > 0 ? Math.max(0, 10 - (timeSpent / timeMax) * 5) : 5;
    
    return {
      success: false,
      technicalScore: Number(testScore.toFixed(1)),
      codeQuality: 5.0,
      problemSolving: Number(timeFactor.toFixed(1)),
      overall: Number(((testScore * 0.5 + 5 * 0.3 + timeFactor * 0.2)).toFixed(1)),
      feedback: `AI grading unavailable: ${error}\n\nBasic evaluation: ${passed}/${total} tests passed.\n\nManual review recommended.`,
      hireRecommendation: 'Review Required',
      aiModel: 'fallback'
    } as PerfectGradingResult;
  }
}

// Test function
if (import.meta.url.endsWith('perfectGrader.ts')) {
...
export async function testSamples() {
  ...
  for (const sample of SAMPLE_PROMPTS) {
    console.log(`\n--- ${sample.name} ---`);
    const result = await gradeSubmission(sample);
